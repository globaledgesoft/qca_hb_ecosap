...
externC void watchdog_reboot(void);
    ...
    do {
        ...
        if( current->state != Cyg_Thread::RUNNING || get_need_reschedule() ) {
            ...
            if (NULL == next) {
                ...
                diag_printf("PANIC: NULL Thread! Rebooting...\n");

#if defined(CYGDBG_SCHED_CRASHDUMP) && (CYGDBG_SCHED_CRASHDUMP == 1)

                current = Cyg_Thread::get_list_head();

                diag_printf("\n%-8s %-2s %-6s %-18s %-10s %-10s %-10s\n",
                                "StackPtr",
                                "ID",
                                "State",
                                "Name",
                ...
                while (current) {
                    ...
                    diag_printf("%-8x %-2d %-2d %-22s 0x%08x 0x%08x 0x%08x\n",
                                current->stack_ptr, current->unique_id,
