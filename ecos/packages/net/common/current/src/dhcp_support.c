...
int dhcp_bind( void )
{
    ...
    if (eth0_up
        && DHCPSTATE_FAILED != eth0_dhcpstate )
            ...
            eth0_up = do_dhcp(WAN_interface, &eth0_bootp_data, &eth0_dhcpstate, &eth0_lease);
#endif            
#endif
#endif
    }
...
int dhcp_halt( void )
{
    ...
    if ( eth0_up
        ...
        do_dhcp_down_net(WAN_interface, &eth0_bootp_data, &eth0_dhcpstate, &eth0_lease);
    }
    eth0_up = false;
#endif
#endif
...
int dhcp_release( void )
{
    ...
    if (eth0_up)
        ...
        do_dhcp_release(WAN_interface, &eth0_bootp_data, &eth0_dhcpstate, &eth0_lease);
#endif
   }
    if (!strcmp(WAN, "eth1")) {
...
#define CYGPKG_NET_DHCP_THREAD_STACK_SIZE 0x4000
