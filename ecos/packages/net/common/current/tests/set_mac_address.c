...
set_mac_address( const char *interface, char *mac_address )
{
    ...
    NET_INFO( "Mac addr %02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    NET_INFO( "%s ioctl(SIOCSIFHWADDR) succeeded\n", interface );
...
{
    int results = 0;
    NET_INFO("Start set_mac_address\n");
#ifdef CYGHWR_NET_DRIVER_ETH0
#ifdef DO_SET_ETH0
   NET_DEBUG("Setting MAC of eth0 to %02x:%02x:%02x:%02x:%02x:%02x\n",
...
#ifdef DO_SET_ETH1
    NET_DEBUG("Setting MAC of eth1 to %02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    if ( 0 == results )
        NET_INFO( "**** Did not set any MAC addresses ****\n" );

    NET_INFO("Init Network Interfaces\n");
