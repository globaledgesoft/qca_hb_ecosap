...
dump_core(char **ptr, void *param)
{
    ...
    case 6:
        ...
        return 0;
    default:
        NET_ERROR("Bad core dump phase: %d\n", dp->phase);
        return 0;
    }
}
...
do_core_dump(core_info_t *cp, char *ip)
{
    ...
    if (!dump_ip) {
        NET_ERROR("Can't dump: server IP missing in NVRAM\n");
        return;
    }
    dump_user = nvram_get("dump_user");
    if (!dump_user) {
        NET_ERROR("Can't dump: user name missing in NVRAM\n");
        return;
    }
    dump_pass = nvram_get("dump_pass");
    if (!dump_pass) {
        NET_ERROR("Can't dump: password missing in NVRAM\n");
        return;
    }
...
loader_main(char *cmdline)
{
    ...
    while (1) {
        ...
        case 'v':
            ...
            break;
        default:
            NET_WARNING("Invalid return from getopt: %d\n", i);
        }
    }
    if (optind < argc) {
    ...
    if (!init_net(eth0_name, &eth0_bootp_data)) {
        NET_ERROR("Network initialization failed for eth0\n");
    }
#endif
#ifdef CYGHWR_NET_DRIVER_ETH1
    if (!parse_eth_addrs(nvram_get("net1_ip"), nvram_get("net1_mask"), 
                         eth1_ip, eth1_netmask, eth1_broadcast)) {
        NET_ERROR("Can't set up ethernet 'net1' - I give up!\n");
        return 1;
    }
    ...
    if (!init_net(eth1_name, &eth1_bootp_data)) {
        NET_ERROR("Network initialization failed for eth1\n");
    }
#endif
