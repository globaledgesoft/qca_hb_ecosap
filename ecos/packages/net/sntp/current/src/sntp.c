...
#define LINK_LOCAL 				0x02
#define SITE_LOCAL 				0x05
#define GLOBAL     				0x0e
...
/* SNTP multicast interfaces */
#define INTERFACE0 				"eth0"
#define INTERFACE1 				"eth1"
...
static void sntp_fn(cyg_addrword_t data)
{
   ...
   */
	...
	// Link-Local
	mreq.ipv6mr_multiaddr.s6_addr[1] = LINK_LOCAL;
	mreq.ipv6mr_interface = if_nametoindex(INTERFACE0);
	if (mreq.ipv6mr_interface != 0 ) {
		ret = setsockopt(fd6, IPPROTO_IPV6, IPV6_JOIN_GROUP, &mreq, sizeof(mreq));
		if (ret < 0) {
			perror("[SNTP]setsockopt:IPV6_JOIN_GROUP:Link-Local");
			goto done;
		}
	}
#endif
	...
	// Link-Local
	mreq.ipv6mr_multiaddr.s6_addr[1] = LINK_LOCAL;
	mreq.ipv6mr_interface = if_nametoindex(INTERFACE1);
	if (mreq.ipv6mr_interface != 0 ) {
		ret = setsockopt(fd6, IPPROTO_IPV6, IPV6_JOIN_GROUP, &mreq, sizeof(mreq));
		if (ret < 0) {
			perror("[SNTP]: setsockopt(IPV6_JOIN_GROUP) Link-Local eth1:");
			goto done;
		}
	}
#endif
	...
	// Site-Local
	mreq.ipv6mr_multiaddr.s6_addr[1] = SITE_LOCAL;
	ret = setsockopt(fd6, IPPROTO_IPV6, IPV6_JOIN_GROUP, &mreq, sizeof(mreq));
	if (ret < 0) {
		perror("[SNTP]: setsockopt(IPV6_JOIN_GROUP) Site-Local:");
		goto done;
	}

	// Global
	mreq.ipv6mr_multiaddr.s6_addr[1] = GLOBAL;
	ret = setsockopt(fd6, IPPROTO_IPV6, IPV6_JOIN_GROUP, &mreq, sizeof(mreq));
	if (ret < 0) {
		perror("[SNTP]: setsockopt(IPV6_JOIN_GROUP) Global:");
		goto done;
	}
  ...
  while (1) {
    ...
    if (NextTimeUpdate == 0 && (sntp_num_servers > 0))
    {
        ...
        for (i = 0; i < sntp_num_servers; i++)
        {
            ...
            {
                ret = sendto(fd, &ntp_pkt, sizeof(ntp_pkt), 0,
                    &sntp_servers[i], sntp_servers[i].sa_len);
#ifdef CYGPKG_NET_INET6
            } else if (sntp_servers[i].sa_family == AF_INET6) {
                ret = sendto(fd6, &ntp_pkt, sizeof(ntp_pkt), 0,
                    &sntp_servers[i], sntp_servers[i].sa_len);
#endif
            }
    ...
    if (FD_ISSET(fd, &readfds)) {
      ret = recvfrom(fd, &ntp_pkt, sizeof(ntp_pkt), 0,
					(struct sockaddr *)&new_srv.addr, &len);
    }
    ...
    if (FD_ISSET(fd6, &readfds)) {
      ret = recvfrom(fd6, &ntp_pkt, sizeof(ntp_pkt), 0,
					(struct sockaddr *)&new_srv.addr, &len);
    }

	if (ret < 0) {
		perror("[SNTP]: recvfrom6:");
		goto done;
	}
#endif
	...
	if (new_srv.version < VERSION_3  || new_srv.version > VERSION_4) {
