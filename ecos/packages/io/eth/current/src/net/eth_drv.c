...
void eth_drv_send(struct ifnet *);
...
eth_drv_init(struct eth_drv_sc *sc, unsigned char *enaddr)
{
    ...
    ether_ifattach(ifp);
#endif
	diag_printf("Initializing interface %s\n", ifp->if_name);
	if(!strcmp(ifp->if_name,"eth")) {
		if(!ppp_flag) {
			sc_Address = (uint32_t *)sc;
			ppp_flag =1;
			diag_printf("ether_drv_init function calling cyg_ppp_pppattach sc address %08x and typecast addr %08x\n",sc,sc_Address);
		}
	}
...
// DEBUG
    ...
    case SIOCGIFHWADDR:
        ...
        if ((sc->funs->control) (sc, cmd, ifr, sizeof(struct ifreq))) {
            error = EINVAL;
        }
    ...
    case SIOCSIFHWADDR:
        ...
        bcopy(&ifr->ifr_hwaddr.sa_data, &sc->sc_arpcom.ac_enaddr,
              ETHER_ADDR_LEN);
    ...
    case SIOCGIFSTATSUD:
        ...
        if ((sc->funs->control) (sc, (cmd == SIOCGIFSTATS)
                                 ...
                                 : ETH_DRV_GET_IF_STATS_UD, data, 0)) {
            error = EINVAL;
        }
        break;
#endif // SIOCGIFSTATS
    ...
    case SIOCSIFFLAGS:
		if ( (IFF_PROMISC & ifp->if_flags) != 0) {
			if ( sc && sc->funs && sc->funs->control )
				(sc->funs->control)(sc,IFF_PROMISC,IFF_PROMISC,1);
		}
		if ( (IFF_ALLMULTI & ifp->if_flags) != 0) {
			if ( sc && sc->funs && sc->funs->control )
				(sc->funs->control)(sc,IFF_ALLMULTI,IFF_ALLMULTI,1);
		}
...
#endif
                if ((LLADDR((struct sockaddr_dl *)ifma->ifma_addr)[0] & 0x01) ==
                    0) {
#ifdef DEBUG
...
#endif
    case SIOCGMACQOSIF:
        if ((sc->funs->control) (sc, cmd, data, 0) < 0)
            error = EINVAL;
        break;
    ...
    case SIOCHWIF:
        ...
        break;
    default:
        if ((strncmp(ifp->if_name, "eth", 3))
            && (strncmp(ifp->if_name, "br", 2))
				&& (strncmp(ifp->if_name, "ppp", 3))) {
...
eth_drv_send(struct ifnet *ifp)
{
    ...
    while ((sc->funs->can_send) (sc) > 0) {
        ...
        if (sg_len) {
            (sc->funs->send) (sc, sg_list, sg_len, total_len,
                              (unsigned long)m0);
	    }
    ...
    // then check that this really is a "sc"
    {
        ...
        for (t = &__NETDEVTAB__[0]; t != &__NETDEVTAB_END__; t++)
            if (((struct eth_drv_sc *)t->device_instance) == sc)
                break;          // found it
        CYG_ASSERT(t != &__NETDEVTAB_END__,
                   "eth_drv_dsr: Failed to find sc in NETDEVTAB");
    }
#endif // Checking code
...
#if defined(CYGDBG_HAL_DEBUG_GDB_CTRLC_SUPPORT)
            was_ctrlc_int =
                HAL_CTRLC_CHECK((*sc->funs->int_vector) (sc), (int)sc);
