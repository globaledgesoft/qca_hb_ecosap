...
find_dev(cyg_flashaddr_t addr, int *stat)
{
    ...
    if (!((addr >= cyg_flashdevtab[0].start)
          && (addr <= cyg_flashdevtab[0].end))) {
...
flash_sort_and_check(void)
{
    ...
    do {
        ...
        for (dev = flash_head, previous_next = &flash_head;
             dev->next; previous_next = &dev->next, dev = dev->next) {
...
cyg_flash_init(cyg_flash_printf * pf)
{
    ...
    CYG_ASSERT(&(cyg_flashdevtab[CYGHWR_IO_FLASH_DEVICE]) ==
               &cyg_flashdevtab_end, "incorrect number of flash devices");
    ...
    for (dev = &cyg_flashdevtab[0]; dev != &cyg_flashdevtab_end; dev++) {
...
#ifdef CYGDBG_USE_ASSERTS
        {
            ...
            for (i = 0; i < dev->num_block_infos; i++) {
                addr +=
                    dev->block_info[i].block_size * dev->block_info[i].blocks;
            }
...
__externC int
cyg_flash_set_printf(const cyg_flashaddr_t flash_base, cyg_flash_printf * pf)
{
    struct cyg_flash_dev *dev;
...
__externC int
cyg_flash_get_info_addr(const cyg_flashaddr_t flash_base,
...
cyg_flash_mutex_lock(const cyg_flashaddr_t from, size_t len)
{
    ...
    if (dev) {
        ...
        if (len > (dev->end + 1 - from)) {
            stat =
                cyg_flash_mutex_lock(dev->end + 1, len - (dev->end + 1 - from));
                ...
                size_t len, cyg_flashaddr_t * err_address)
{
    ...
    if (len > (dev->end + 1 - flash_base)) {
        return cyg_flash_erase(dev->end + 1,
                               len - (dev->end + 1 - flash_base), err_address);
    }
    return CYG_FLASH_ERR_OK;
}
                  ...
                  size_t len, cyg_flashaddr_t * err_address)
{
    ...
    CHATTER(dev, "... Program from %p-%p to %p: ", ram_base,
	...
	// TODO: Invalidate only the length written?
        stat = dev->funs->flash_program(dev, addr, ram, this_write);
		HAL_DCACHE_INVALIDATE(addr, SECTOR_SIZE);
#ifdef CYGSEM_IO_FLASH_VERIFY_PROGRAM
        if (CYG_FLASH_ERR_OK == stat)   // Claims to be OK
            if (0 && !dev->funs->flash_read
                && memcmp((void *)addr, ram, this_write) != 0) {
                 ...
                 size_t len, cyg_flashaddr_t * err_address)
{
    ...
    if (!dev)
        return stat;
    if (!dev->funs->flash_block_unlock)
        return CYG_FLASH_ERR_INVALID;
...
int
cyg_flash_devfn_unlock_nop(struct cyg_flash_dev *dev,
