...
#undef CACHE_MM_BIT_ENABLE

#define HAL_MIPS_CONFIG_MM 18
...
void hal_c_cache_init(unsigned long config1_val)
{
  ...
  val &= ~HAL_MIPS_CONFIG_KSEG0_CACHE_MASK;
  val |= CYGPKG_HAL_MIPS_KSEG0_CACHE_CONFIG;

#ifdef CACHE_MM_BIT_ENABLE
  val |= (1 << HAL_MIPS_CONFIG_MM);
#endif /*CACHE_MM_BIT_ENABLE*/
...
#undef HAL_DCACHE_START_ADDRESS
#undef HAL_DCACHE_END_ADDRESS
#undef HAL_ICACHE_START_ADDRESS
#undef HAL_ICACHE_END_ADDRESS
...
((CYG_ADDRESS)(((CYG_ADDRESS)(_addr_) + (_asize_) - 1) & ~(HAL_DCACHE_LINE_SIZE-1)))
...
void HAL_DCACHE_INVALIDATE(CYG_ADDRESS _base_, CYG_ADDRESS _asize_)
{
...
void HAL_DCACHE_FLUSH(CYG_ADDRESS _base_, CYG_ADDRESS _asize_)
{
    register CYG_ADDRESS _addr_  = HAL_DCACHE_START_ADDRESS(_base_);
    register CYG_ADDRESS _eaddr_ = HAL_DCACHE_END_ADDRESS(_base_, _asize_);
    ...
    for( ; _addr_ <= _eaddr_; _addr_ += HAL_DCACHE_LINE_SIZE) {
        asm volatile (" cache %0, 0(%1)" \
                    : \
                    : "I" (HAL_MIPS_CACHE_HIT_WRITEBACK_INV_D), \
                      "r"(_addr_));
    }
}

void HAL_DCACHE_STORE(CYG_ADDRESS _base_, CYG_ADDRESS _asize_)
{
    register CYG_ADDRESS _addr_  = HAL_DCACHE_START_ADDRESS(_base_);
    register CYG_ADDRESS _eaddr_ = HAL_DCACHE_END_ADDRESS(_base_, _asize_);
    ...
    for ( ; _addr_ <= _eaddr_; _addr_ += HAL_DCACHE_LINE_SIZE) {
        asm volatile (" cache %0, 0(%1)"\
                    : \
                    : "I" (HAL_MIPS_CACHE_HIT_WRITEBACK_D), \
                      "r"(_addr_));
    }
}
